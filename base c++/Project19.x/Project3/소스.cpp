...
int main() {
	...
	if (true) {
		...
		//작동되는 과정을 보면 어떤 쓰레드에서 메인 함수가 작동합니다. 
		//그리고 위 코드로 다른 쓰레드를 실행시킵니다.
		//그러면 새로 생긴 쓰레드는 자기를 실행시킨 쓰레드와 상관없이 작동합니다.
	...
	if (false) {
		...
		//출력이 겹치는걸 해결하기 위해 mutex를 사용했습니다. 자세한 원리는 다음강의에서 말씀드리겠습니다.
